...
import pandas as pd
from models import cardio_predict_svm
...
SVM_API_URL = 'https://cardio-vascular-disease-prediction-v6sw.onrender.com/cardio-predict-svm'

features, target, X, Y, scaler, model, Y_pred, cm, cr = cardio_predict_svm()
...
if st.button('Predict Cardio'):
    ...
    try:
        response = requests.post(SVM_API_URL, json=payload)

        if response.status_code == 200:

            result = response.json()

            if result['svm_prediction'] == 0:
...
ax.set_title('Actual Cardio vs. Predicted Cardio')
